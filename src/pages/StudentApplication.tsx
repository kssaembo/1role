...
export default function StudentApplication() {
  ...
  const handleSubmit = async () => {
    if (!selectedStudent || preferences.length < 3 || !commitment.trim()) return;
    setIsSubmitting(true);
    try {
      // 1. Check if application already exists for this student
      const q = query(collection(db, `users/${teacherId}/applications`), where('studentId', '==', selectedStudent.id));
      ...
      await addDoc(collection(db, `users/${teacherId}/applications`), {
      ...
      for (const roleId of preferences) {
        const roleRef = doc(db, `users/${teacherId}/roles`, roleId);
